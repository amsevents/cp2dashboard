...
async function fetchAging() {
      try {
        ...
        const data = await alvysPost(ALVYS_ENDPOINTS.invoicesSearch, {
              ...
              console.log("Alvys /invoices/search Total:", data.Total);

        const rawInvoices = data.Items || data.items || data.results || data.invoices || (Array.isArray(data) ? data : []);

        const typeCounts = {};
              for (const inv of rawInvoices) {
                        const key = `${inv.Type || "unknown"}${inv.IsVoided ? " (voided)" : ""}`;
                        typeCounts[key] = (typeCounts[key] || 0) + 1;
              }
              console.log("Alvys /invoices/search Type breakdown:", JSON.stringify(typeCounts));
        ...
        for (const inv of rawInvoices) {
                  ...
                  if (inv.Type && inv.Type !== "LoadInvoice") continue;

                const balance = inv.RemainingBalance?.Amount ?? 0;
                  if (!balance || balance <= 0) continue;

                const dateStr = inv.DueDate ?? inv.InvoicedDate;
                  let ageDays = 0;
                  if (dateStr) {
                ...
                const entry = {
                            load_id: inv.Loads?.[0]?.LoadNumber ?? "—",
